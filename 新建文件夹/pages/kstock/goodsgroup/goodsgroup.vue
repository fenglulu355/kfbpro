<template>
	<view class="editinfo">
		<view class="top"><topback :type="'two'" :topback="'商品分组'" :iscenter="true" :iswhite="true" @change="tobacks"></topback></view>
		<view class="content">
			<view class="box">
...
export default {
	...
	methods: {
		...
		del(item) {
			...
			uni.showModal({
				...
				success: function(res) {
					if (res.confirm) {
						that.$request('/goods/store/goods/group/delete', { groupId: item.groupId, storeId: that.storeId }, 'delete', that.authorization, 'application/json').then(
							res => {
								if (res.kfbCode == 200) {
									uni.showToast({
										title: '删除成功'
		...
		send(data) {
			this.$request('/goods/store/goods/group/save', data, 'post', this.authorization).then(res => {
				if (res.kfbCode == 200) {
					uni.showToast({
						title: '编辑成功'
					});
